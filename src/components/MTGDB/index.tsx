...
export async function getDeckCards(
  ...
  deckName: string,
  category?: string
): Promise<CardsTableType[]> {
  let _where: { [key: string]: string } = { name: deckName };
  if (category) {
    _where['category'] = category;
  }

  let _deckCardsId: number[] = (await db.decks.where(_where).toArray()).map(
    (d) => d.card_id
  );

  let res: CardsTableType[] = [];
  for (let id of _deckCardsId) {
    let _card = await db.cards.get(id);
    if (_card) {
      res.push(_card);
    }
  }
